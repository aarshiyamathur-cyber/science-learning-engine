...
### BL-053 — Write the Plate Tectonics lesson, assessment, and PlateBoundaryExplorer widget

- **Description:** `sci-y7-plate-tectonics` (added as an architecture prerequisite in Sprint 15) had zero lesson content, zero questions, and no interactive widget. Write it from scratch per the visual-learning standard: Earth's layered structure (crust/mantle/core), tectonic plates moving on the solid, slowly-flowing mantle via convection, the three boundary types and their distinct effects, why earthquakes/volcanoes concentrate at boundaries, and evidence for plate tectonics.
- **Dependencies:** none
- **Priority:** P0
- **Status:** Done
- **Acceptance criteria:** New `app/components/widgets/PlateBoundaryExplorer.tsx` — a standalone, curriculum-agnostic widget with no required props, offering 3 tappable boundary types (Convergent/Colliding, Divergent/Separating, Transform/Sliding Past), each reveal showing a distinct arrow-direction visual matching the plates' motion, a movement description, an effect description, and a real-world example, so the "every boundary produces the same effect" misconception is corrected visually, not just stated in text; new `curriculum/lessons/lesson-plate-tectonics-intro.yaml` (illustration → explanation → explanation → illustration → example → interactive `plate-boundary-explorer` → explanation → explanation → 5 questions → summary) and `curriculum/assessments/assessment-plate-tectonics-quiz.yaml`; lesson references illustration ids `plate-tectonics-hero` and `plate-boundary-types` and widget id `plate-boundary-explorer` (registration in `LessonPlayer`'s registries left for the QA worker, per directive); 5 questions with hints and explanations covering the concept's learning objectives and directly targeting both misconceptions ("mantle-is-liquid-plates-float", "all-boundaries-same-effect"); `validate:curriculum`/`typecheck`/`lint`/`vitest run` all pass; new unit tests for the widget.
- **Estimate:** Medium
- **Notes:** Every explanation/example step body is 2-4 sentences per the visual-learning standard. q-plate-tectonics-02 targets the mantle-is-liquid-plates-float misconception directly (a student claim that the mantle is a liquid ocean of magma that plates float on top of). q-plate-tectonics-03 targets the all-boundaries-same-effect misconception directly (distinguishing which boundary type mainly causes earthquakes with little to no volcanic activity). Delegated to and completed by an OpenClaw headless worker in a dedicated worktree. Built in parallel with a second worker producing the `plate-tectonics-hero` and `plate-boundary-types` illustrations (`app/components/icons/`) — neither that directory nor `LessonPlayer.tsx` touched here; the concept file, knowledge graph, and topic sequence were already wired by a prior Sprint 15 architecture-prerequisite commit, so `curriculum/concepts/`, `curriculum/graph/knowledge-graph.yaml`, and `app/page.tsx` were not touched here either; registry wiring left for the QA worker, per the same pattern as BL-051/CE-007 and BL-052/CE-008. `typecheck`, `lint`, and `vitest run` (130/130 tests, 20 files — 125 baseline + 5 new widget tests) all pass. **Reviewed by OpenClaw QA worker** (headless, independent review + live browser click-through of every lesson step: hero illustration → both crust/mantle explanations → mid-lesson boundary-types illustration → Himalayas example → the `PlateBoundaryExplorer` widget (all 3 boundary types tapped — convergent, divergent, transform — each showing a distinct arrow-direction visual, movement, effect, and real-world example) → earthquakes/volcanoes-concentrate-at-boundaries explanation → continental-drift-evidence explanation → all 5 questions, one answered incorrectly on purpose and one hint revealed → summary → finish, on a scratch test learner via the "Testing as" control (with `completedLessons` for the 12 prerequisite lessons seeded directly via `node:sqlite` so the lesson showed as Ready immediately), never touching Aarshiya's real profile. Confirmed XP increased 0 → 50 and the lesson showed Completed afterward. No console errors; no horizontal overflow at 375px, on the topic list or inside the lesson. Content re-verified directly against source: mantle correctly described as solid rock that flows via convection (not liquid), plate speeds described only qualitatively ("a few centimetres a year", no numeric velocity/magnitude detail), three boundary types with genuinely distinct effects; both misconceptions correctly and directly targeted with the right directionality; every question has both a hint and an explanation. Scratch learner rows deleted from `learner_profiles`/`mastery_states`/`attempt_records` after the test). **Accepted: Yes.**
...
- **Estimate:** Small
- **Notes:** Built in a dedicated worktree, scoped strictly to `app/components/icons/`, in parallel with a second worker writing the actual "Plate Tectonics" lesson content — neither `curriculum/`, `LessonPlayer.tsx`, nor `ContinueLearningScreen.tsx` touched here; wiring into the `ILLUSTRATION_REGISTRY` and the lesson content itself (ids `plate-tectonics-hero` and `plate-boundary-types`) is left for the QA worker, per the same pattern as CE-006/CE-007/CE-008. `PlateTectonicsHeroIllustration` nests a small rose core circle inside a larger amber mantle circle, both centred at (90,60), with four separate 54-radius stroke arcs (alternating sky and emerald, 70 degrees each with 20-degree gaps) wrapping the outside as distinct crust plate segments. `PlateBoundaryTypesIllustration` splits the canvas into three dashed-divider panels: the convergent panel shows two plate blocks with an amber uplift triangle between them and arrows pointing toward each other; the divergent panel shows two plate blocks with an amber wedge filling the gap between them and arrows pointing apart; the transform panel shows two same-height plate blocks against a dashed vertical fault line with vertical arrows on each block pointing in opposite directions, parallel to the boundary rather than toward or away from it. Delegated to and completed by an OpenClaw headless worker; `typecheck`, `lint`, and `vitest run` (127/127 tests, 19 files — 125 baseline + 2 new render tests) all pass. **Reviewed by OpenClaw QA worker** (headless, independent review + live browser click-through — read the SVG source directly and confirmed rendering live in the lesson flow. `PlateTectonicsHeroIllustration` is interpretable without a caption as an Earth cross-section: a small core nested inside a larger mantle disc, wrapped by a broken ring of separate arc segments reading as distinct rigid crust plates over the continuous layers beneath. `PlateBoundaryTypesIllustration` is interpretable without a caption: three panels distinguished entirely by arrow direction and the presence/absence of a bump — converging arrows into an uplifted ridge, diverging arrows around a filled gap, and parallel opposite-direction arrows with neither ridge nor gap — so the "every boundary looks the same" misconception has something concrete to contrast against. No text baked into either SVG, existing hex palette only). **Accepted: Yes.** Registry wiring done by the QA worker per directive; merged to master alongside BL-053.

## Sprint 16

### BL-054 — Write the Solar System and Universe lesson, assessment, and EarthSunMoonExplorer widget

- **Description:** `sci-y7-solar-system` (added as an architecture prerequisite in Sprint 16) had zero lesson content, zero questions, and no interactive widget. Write it from scratch per the visual-learning standard: the Solar System's structure (Sun, eight planets — rocky inner vs. gas giant outer, moons, asteroids, comets — held by the Sun's gravity), Earth's rotation causing day/night, Earth's tilted axis and orbit causing the seasons, the Moon's phases, and the Solar System's place within the Milky Way galaxy and the wider Universe.
- **Dependencies:** none
- **Priority:** P0
- **Status:** Done
- **Acceptance criteria:** New `app/components/widgets/EarthSunMoonExplorer.tsx` — a standalone, curriculum-agnostic widget with no required props, offering two modes: Seasons (four tappable Southern-Hemisphere positions — Summer/December, Autumn/March, Winter/June, Spring/September — each revealing a distinct orbit-diagram visual of Earth's axial tilt toward, away from, or side-on to the Sun, plus the resulting day length) and Moon Phases (four tappable phases — New, First Quarter, Full, Third Quarter — each revealing a distinct shaded-circle visual of exactly how much of the Moon's sunlit half is visible from Earth at that orbital position), so both misconceptions are corrected visually, not just stated in text; new `curriculum/lessons/lesson-solar-system-intro.yaml` (illustration → explanation → explanation → illustration → example → interactive `earth-sun-moon-explorer` → explanation → explanation → 5 questions → summary) and `curriculum/assessments/assessment-solar-system-quiz.yaml`; lesson references illustration ids `solar-system-hero` and `cosmic-scale` and widget id `earth-sun-moon-explorer` (registration in `LessonPlayer`'s registries left for the QA worker, per directive); 5 questions with hints and explanations covering the concept's learning objectives and directly targeting both misconceptions (`seasons-caused-by-distance-from-sun`, `moon-phases-are-earths-shadow`); `validate:curriculum`/`typecheck`/`lint`/`vitest run` all pass; new unit tests for the widget.
- **Estimate:** Medium
- **Notes:** Every explanation/example step body is 2-4 sentences per the visual-learning standard, and the syllabus scope was deliberately kept to categorical planet structure and qualitative day/night, seasons, and Moon-phase mechanisms — no orbital mechanics, numeric distances/periods, or eclipse mechanics beyond the one-sentence eclipse/phase distinction. q-solar-system-02 targets the seasons-caused-by-distance-from-sun misconception directly (a student claim that Earth is closer to the Sun in summer and farther in winter). q-solar-system-04 targets the moon-phases-are-earths-shadow misconception directly (a student claim that Earth's shadow covers part of the Moon to cause its phases). Delegated to and completed by an OpenClaw headless worker in a dedicated worktree. Built in parallel with a second worker producing the `solar-system-hero` and `cosmic-scale` illustrations (`app/components/icons/`) — neither that directory nor `LessonPlayer.tsx` touched here; the concept file, knowledge graph, and topic sequence were already wired by a prior Sprint 16 architecture-prerequisite commit, so `curriculum/concepts/`, `curriculum/graph/knowledge-graph.yaml`, and `app/page.tsx` were not touched here either; registry wiring left for the QA worker, per the same pattern as BL-052/CE-008 and BL-053/CE-009. The `EarthSunMoonExplorer` widget's Seasons mode is explicitly written for the Southern Hemisphere (Aarshiya is in Australia), so December is Summer and June is Winter, matching the lesson's NSW-summer/NSW-winter example step. `typecheck`, `lint`, and `vitest run` (140/140 tests, 21 files — 132 baseline + 8 new widget tests) all pass. Self-reviewed only at this stage — content read in full against the concept file's five learning objectives and both misconceptions, but not yet independently reviewed or exercised in a live browser by a QA worker. **Accepted: Pending QA review.**
### CE-010 — Solar System & Universe illustrations

- **Description:** Per the permanent visual-learning standard, every lesson needs a hero illustration plus supporting diagrams rendered inside the lesson flow itself via the `illustration` lesson-step type. Added two new hand-authored SVG illustrations for the in-flight "The Solar System and Universe" lesson (Sprint 16, Earth and Space strand), matching the existing inline-SVG style (see `app/components/icons/Illustrations.tsx`).
- **Dependencies:** none
- **Priority:** P1
- **Status:** Done
- **Acceptance criteria:** `SolarSystemHeroIllustration` (the Sun at centre inside a set of concentric orbit rings, holding four small warm-toned rocky planets close in, a faint dotted asteroid-belt ring, then four larger cool-toned gas-giant planets — one carrying a ring — farther out, so the rocky-vs-gas-giant grouping reads as two visually distinct clusters around one star) and `CosmicScaleIllustration` (a tiny Solar System icon nested at the core of a spiral Milky Way galaxy, itself one of several small spiral galaxies scattered inside a faint dashed boundary, teaching the Solar System-within-a-galaxy-within-the-Universe hierarchy) added to `app/components/icons/Illustrations.tsx` and exported from `app/components/icons/index.ts` under those exact names; hand-authored inline SVG only, existing hex palette, no external assets, no text baked into either SVG; each is interpretable without its caption; render tests added; `typecheck`/`lint`/`vitest run` all pass.
- **Estimate:** Small
- **Notes:** Built in a dedicated worktree, scoped strictly to `app/components/icons/`, in parallel with a second worker writing the actual "The Solar System and Universe" lesson content — neither `curriculum/`, `LessonPlayer.tsx`, nor `ContinueLearningScreen.tsx` touched here; wiring into the `ILLUSTRATION_REGISTRY` and the lesson content itself (ids `solar-system-hero` and `cosmic-scale`) is left for the QA worker, per the same pattern as CE-007/CE-008/CE-009. `SolarSystemHeroIllustration` draws nine concentric orbit ellipses around the Sun (a nested amber/rose disc pair), with the four rocky planets (amber/rose) placed on the four innermost rings, a dashed asteroid-belt ring between the rocky and gas-giant groups, and the four gas giants (sky/violet/emerald, one wearing a rotated ring) placed on the four outermost rings — rocky planets stay warm-toned and small, gas giants stay cool-toned and large, matching the teaching distinction rather than real orbital scale. `CosmicScaleIllustration` centres a two-armed spiral galaxy (a violet core with sky spiral-arm strokes) at the middle of a faint dashed "Universe" boundary circle, with a miniature Sun-plus-three-planet-dot Solar System icon nested directly inside that galaxy's core, and three smaller spiral galaxies (rose, emerald, amber) scattered elsewhere inside the same boundary — so the nested "Solar System → galaxy → Universe" hierarchy reads without a caption. Delegated to and completed by an OpenClaw headless worker; `typecheck`, `lint`, and `vitest run` (134/134 tests, 20 files — 132 baseline + 2 new render tests) all pass.
